...
impl FragmentManager(){
    ...
    pub fn new(rId : i32, fId : i32, t : i32, tmp : Path){
        FragmentManager{
            serverIP : String :: new(),
            serverPort : -1,
            toServer : None,
    ...
    pub fn submit(&mut self) -> bool {
        ...
        if let Ok(connect_socket) = TcpStream::connect((&self.server_ip[..], self.control_port)) {
            self.to_server = Some(connect_socket);//忽略了setKeepAlieve和setsoTimeout，未找到rust中对应的长连接和超时连接的处理函数
    ...
    pub fn init(&mut self, f : Path, ip : String, port : i32) {
